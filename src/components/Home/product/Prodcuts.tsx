  ...
  return (
    <div className="my-6">
      ...
      {products.map((product) => (
        // to={`/detail/${product.id}`} state={{ product: product }}
        <Link
          to={`/detail/${product.id}`}
          state={{ product: product }}
          key={product.id}
        >
          <div className="shadow-sm border rounded-md p-2  flex my-5 hover:scale-105 transition-all ease-linear duration-200 delay-75 cursor-pointer">
